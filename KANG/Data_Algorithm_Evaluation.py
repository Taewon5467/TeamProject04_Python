...
df_ml = pd.get_dummies(df, columns=['요일', '월', '시간'], drop_first=False)
...
sns.barplot(data=comparison_df, x='Model', y='R2_Score', hue='Model', palette='viridis', ax=ax1, width=0.4, legend=False)
...
ax1.set_xticklabels(comparison_df['Model'], rotation=10)
...
sns.barplot(data=comparison_df_rmsle, x='Model', y='RMSLE', hue='Model', palette='magma', ax=ax3, width=0.4, legend=False)
...
ax3.set_xticks(range(len(comparison_df_rmsle)))
ax3.set_xticklabels(comparison_df_rmsle['Model'], rotation=10)
